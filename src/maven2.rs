...
fn parse_integer_token(token: &str) -> Result<u32, String> {
    if token.chars().count() > 1 && token.starts_with('0') {
        ...
        token
            .parse::<u32>()
            .map_err(|_| "Number is invalid".to_string())
    }
}
        ...
        if result == Ordering::Equal {
            if let Some(qualifier) = self.qualifier() {
                if let Some(other_qualifier) = other.qualifier() {
                    ...
                    if qualifier_count > other_qualifier_count
                        && qualifier.starts_with(other_qualifier)
                    {
                        // here, the longer one that otherwise match is considered older
                        result = Ordering::Less;
                    } else if qualifier_count < other_qualifier_count
                        && other_qualifier.starts_with(qualifier)
                    {
                        // here, the longer one that otherwise match is considered older
...
mod tests {
    ...
    fn check_version_parsing(
        ...
        assert_eq!(
            build_number,
            actual.build_number(),
            "{} check build number version",
            parsed
        );
        assert_eq!(
            qualifier,
            actual.qualifier(),
            "{} check qualifier version",
            parsed
        );
    }
    ...
    fn assert_version_older(left: &str, right: &str) {
        assert!(
            new_artifact_version(left).cmp(&new_artifact_version(right)) == Ordering::Less,
            "{} should be older than {}",
            left,
            right
        );
        assert!(
            new_artifact_version(right).cmp(&new_artifact_version(left)) == Ordering::Greater,
    ...
    fn assert_version_equal(left: &str, right: &str) {
        assert!(
            new_artifact_version(left).cmp(&new_artifact_version(right)) == Ordering::Equal,
            "{} should be equal to {}",
            left,
            right
        );
        assert!(
            new_artifact_version(right).cmp(&new_artifact_version(left)) == Ordering::Equal,
            "{} should be equal to {}",
            right,
            left
        );
